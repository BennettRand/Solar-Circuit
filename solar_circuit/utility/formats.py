...
def uint8(registers, order=1):
	...
	data_str = modbus_string(registers, order)
	return float(struct.unpack("!xB", data_str)[0])
...
def uint16(registers, order=1):
	if len(registers) != 1:
		raise ValueError("Incorrect number of registers.")
	data_str = modbus_string(registers, order)
	return float(struct.unpack("!H", data_str)[0])
...
def uint32(registers, order=1):
	if len(registers) != 2:
		raise ValueError("Incorrect number of registers.")
	data_str = modbus_string(registers, order)
	return float(struct.unpack("!I", data_str)[0])
...
def uint64(registers, order=1):
	if len(registers) != 4:
		raise ValueError("Incorrect number of registers.")
	data_str = modbus_string(registers, order)
	return float(struct.unpack("!Q", data_str)[0])
